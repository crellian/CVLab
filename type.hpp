...
template<class T> _Vector<T>::_Vector(int _len) :len(_len) {data=new T[_len]; for(int i=0; i<_len; i++) data[i]=0;}
...
template<class T> T& _Vector<T>::operator[] (int idx) const {return *(data+idx);}
