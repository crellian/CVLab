...
public:
    ...
    const T& operator[](int idx) const;                  //For const Object usage
    T& operator[](int idx);
...
public:
    ...
    _Matrix dot(const _Matrix& m) const;
...
private:
    struct Data                                 //reference counting
    {
        Data(const T* initVal);
        ~Datal();
        int refCount;
        T* val;
    };
...
template<class T, class _T> _Matrix<T> operator * (const Matrix<T>& m, _T val);
template<class T, class _T> _Matrix<T> operator * (_T val, const Matrix<T>& m);
