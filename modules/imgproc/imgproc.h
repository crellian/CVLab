#ifndef MODULES_IMGPROC_IMGPROC_H_
#define MODULES_IMGPROC_IMGPROC_H_
#include "types.h"
...
namespace CVLab {
	...
	template<class T> void boxFilter(const _Matrix<T>& src, _Matrix<T>& dst, int ddepth, Size ksize, bool normalize = true, int borderType = 0);
	template<class T> void bilateralFilter(const _Matrix<T>& src, _Matrix<T>& dst, int d, double sigmaColor, double sigmaSpace, int borderType = 0);
	template<class T> void GaussianBlur(const _Matrix<T>& src, _Matrix<T>& dst, Size ksize, double sigmaX, double sigmaY = 0, int borderType = 0);
	...
	template<class T> void dilate(const _Matrix<T>& src, _Matrix<T>& dst, const _Matrix<T>& kernel);
	template<class T> void erode(const _Matrix<T>& src, _Matrix<T>& dst, const _Matrix<T>& kernel);
...
#endif //MODULES_IMGPROC_IMGPROC_H_
