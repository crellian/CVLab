...
namespace CVLab {
	...
	template<class T> void dilate(const _Matrix<T>& src, _Matrix<T>& dst, const Size& kernel)
	{
		int row = src.GetRow();
		int col = src.GetCol();
	...
	template<class T> void erode(const _Matrix<T>& src, _Matrix<T>& dst, const Size& kernel)
	{
		int row = src.GetRow();
		int col = src.GetCol();
		int channel = src.GetChannel();
	...
	template<class T> void Sobel(const Matrix8u& src, Matrix8u& dst, int dx, int dy, int ksize, bool thresholding, bool get_dir, Matrix8u& dir)
	{
		int cha = src.GetChannel();
